...
for url in user_links_ozon:
    ...
    try:
        try: 
            if soup.find('div', {'data-widget': 'error'}).find('h2').get_text(strip=True) == 'Такой страницы не существует':
            #ЛОГИКА ДЛЯ БОТА(пишем, что страница неверно указана)
                continue
        except:
            pass
